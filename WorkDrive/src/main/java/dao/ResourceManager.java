...
public class ResourceManager {
	...
	public static JSONObject addResource(String resourceName, Long parentId, long userId) throws SQLException, IOException {
		...
		if (rs.next()) {
			...
			Resource resource = new Resource(rs.getLong("ResourceId"), rs.getString("ResourceName"), rs.getLong("CreatedTime"), rs.getLong("LastModifiedTime"), rs.getLong("parentId"), rs.getString("TimeZone"), totalFiles, totalFolders, size);
	...
	public static ArrayList<JSONObject> getAllFiles(long folderId, long cursor, int limit) throws SQLException {
		...
		while (result.next()) {
			long createdTime = result.getTimestamp("fileCreateTime").getTime();
		    long modifiedTime = result.getTimestamp("fileEditTime").getTime(); 
