...
public class ResourceManager {
	...
	public static JSONObject addResource(String resourceName, Long parentId, long userId)
		...
		if (parentId != null) {
			QueryHandler.executeUpdate(Queries.ADD_RESOURCE,
					new Object[] { id, resourceName, parentId, userId, currentTime, currentTime });
		} else {
			QueryHandler.executeUpdate(Queries.ADD_RESOURCE_ROOT,
					new Object[] { id, resourceName, userId, currentTime, currentTime });
		}
	...
	public static ArrayList<JSONObject> getAllFiles(long folderId , long userId, long cursor, int limit ) throws SQLException {
		...
		while (result.next()) {
			files.add(new File(result.getString("filename"), result.getLong("fileCreateTime"),
					result.getLong("fileEditTime"), result.getString("Size"), result.getLong("fileId"), timeZone)
					.getFileData());
		}
	...
	public static boolean renameFile(String filename, long fileId) {
		...
		long currentTimeMills = System.currentTimeMillis();
		int res = QueryHandler.executeUpdate(Queries.UPDATE_FILENAME,
