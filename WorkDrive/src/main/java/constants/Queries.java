...
public class Queries {
	...
	public static final String SEARCH_RESOURCES="SELECT * FROM "+RESOURCE_TABLE+" r JOIN "+USERS_TABLE+" u ON u."+USER_ID+"=r."+USER_ID+" WHERE r."+USER_ID+"= ? AND "+PARENT_ID+" IS NOT NULL AND (r."+RESOURCE_NAME+" LIKE CONCAT(?,'%') OR r."+RESOURCE_NAME+" LIKE CONCAT('%',?) OR r."+RESOURCE_NAME+" LIKE CONCAT('%',?,'%'))"+" ORDER BY CASE WHEN r."+RESOURCE_NAME+" LIKE CONCAT(?,'%') THEN 1 WHEN r."+RESOURCE_NAME+" LIKE CONCAT('%',?) THEN 2 WHEN r."+RESOURCE_NAME+" LIKE CONCAT('%',?,'%') THEN 3 END";

	public static final String GET_RESOURCE_BASIC="SELECT "+RESOURCE_ID+", "+RESOURCE_NAME+", "+PARENT_ID+" FROM "+RESOURCE_TABLE+" WHERE "+RESOURCE_ID+" = ? AND  "+RESOURCE_TYPE+" = 'FOLDER' AND "+RESOURCE_STATUS+" = 'active'";
